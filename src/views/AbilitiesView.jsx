...
export default function AbilitiesView() {
  ...
  const handleAbilitySelect = async (ability) => {
    ...
    try {
      const pokemon = await getPokemonByAbility(ability);
    ...
    <div className="space-y-6">
      ...
      {!selectedAbility && !loading && (
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-96 overflow-y-auto border border-purple-500/30 rounded-lg p-4 bg-slate-800/50">
            {filteredAbilities.map((ability) => (
              <button
                key={ability}
                onClick={() => handleAbilitySelect(ability)}
                className="px-4 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all text-sm font-medium capitalize shadow-lg"
              >
                {ability}
              </button>
            ))}
